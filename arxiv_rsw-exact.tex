...
Other examples of chiral crystals include AlPt \cite{schroter}, RhGe (with the energy-separatation between RSWN and the TPFs being $\sim 0.21 $ eV) \cite{bahadur-RhGe}, RhSi \cite{sanchez} Li$_2$Pd$_3$B \cite{law-LiPdB}, Li$_2$Pt$_3$B \cite{law-LiPdB}, and CoSi \cite{sanchez, tang2017_multiple}.
...
\end{align}
%%%%%%%%%%%%%%%%%%%%%
This, in turn, it affects the explicit forms of the Hamilton's equations, which we will discuss In Sec.~\ref{secboltz}.
...
\end{align}
%%%%%%%
as schematically depicted in Fig.~\ref{figdis}(b). Consequently, in addition to affecting the Fermi-Dirac distributions functions, it causes modifications of the group-velocities via adding
%%%%%%%%%%%%%%%%
\begin{align}
...
\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%
In total we have 8 undetermined coefficients so determine, denoted as the set $\lbrace \lambda_s , \, a_s , \, b_s , 
...
\end{align}
Here, $\tau$ is the momentum-independent relaxation time within the RTA framework and the superscipts, ``dr'', ``bc'', and ``$m$'', indicate the Drude (i.e., $B$-independent), BC-only, and OMM-induced contributions, respectively.
